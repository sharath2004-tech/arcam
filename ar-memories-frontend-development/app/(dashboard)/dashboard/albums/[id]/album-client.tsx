...
export default function AlbumDetailPage() {
  ...
  function onPhotosChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setDrafts(prev => [...prev, ...files.map(f => ({ preview: URL.createObjectURL(f), caption: '' }))]);
  }
  ...
  function resetForm() {
    setDrafts([]); setVideoPreview(null); setUploading(false);
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    // All photos share the same videoUrl
    const newPhotos: Photo[] = drafts.map(d => ({ url: d.preview, caption: d.caption, videoUrl: videoPreview }));
    setPhotos(prev => [...prev, ...newPhotos]);
    if (!coverUrl && newPhotos.length > 0) setCoverUrl(newPhotos[0].url);
    setUploading(false);
    resetForm();
    setShowUpload(false);
  }
    ...
    <div className="space-y-8">
      ...
      {photos.length === 0 ? (
        <div className="glass-effect rounded-2xl p-16 flex flex-col items-center text-center">
          ...
          <p className="text-muted-foreground/60 text-sm mt-2">Upload photos and link one video — all photos share the same AR video.</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {photos.map((photo, i) => (
            <div key={i} className="group relative rounded-2xl overflow-hidden aspect-square bg-muted cursor-pointer" onClick={() => setSelected(photo.url)}>
              ...
              {photo.videoUrl && (
                <button onClick={e => { e.stopPropagation(); setPlayingUrl(photo.videoUrl!); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass-effect rounded-2xl p-6 w-full max-w-lg space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-foreground">Add Photos</h2>
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-foreground/70 text-sm font-medium">Photos <span className="text-destructive">*</span></label>
                  {drafts.length > 0 && (
                    <button type="button" onClick={() => photoInputRef.current?.click()} className="text-primary text-xs hover:underline">+ Add more</button>
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {drafts.map((d, i) => (
                      <div key={i} className="relative rounded-xl overflow-hidden aspect-square bg-muted group">
                        ...
                        <button type="button" onClick={() => setDrafts(p => p.filter((_, j) => j !== i))}
                        ...
                        <input
                          type="text"
                          value={d.caption}
                          onChange={e => setDrafts(p => p.map((x, j) => j === i ? { ...x, caption: e.target.value } : x))}
                          placeholder="Caption…"
                ...
                {drafts.length > 0 && <p className="text-muted-foreground text-xs mt-1.5">{drafts.length} photo{drafts.length > 1 ? 's' : ''} selected</p>}
              </div>

              {/* Single shared video */}
              <div>
                <label className="text-foreground/70 text-sm font-medium block mb-1">
                  AR Video <span className="text-muted-foreground font-normal">(one video plays for all photos when scanned)</span>
                </label>
                ...
                {videoPreview ? (
                  <div className="relative rounded-xl overflow-hidden aspect-video bg-black/20">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-10 h-10 rounded-full bg-black/60 flex items-center justify-center"><Film className="w-5 h-5 text-white" /></div>
                    </div>
                    <button type="button" onClick={() => { setVideoPreview(null); if (videoInputRef.current) videoInputRef.current.value = ''; }}
                      className="absolute top-2 right-2 p-1 rounded-lg bg-black/60 text-white"><X className="w-4 h-4" /></button>
                  </div>
                ) : (
                  <button type="button" onClick={() => videoInputRef.current?.click()}
                    className="w-full h-24 rounded-xl border-2 border-dashed border-border hover:border-primary/50 flex flex-col items-center justify-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
              ...
              {uploading && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
                  ...
                  <span className="text-muted-foreground text-sm">Uploading {drafts.length} photo{drafts.length > 1 ? 's' : ''}…</span>
                </div>
              )}

              <div className="flex gap-3 pt-1">
                <Button type="button" variant="outline" onClick={() => { setShowUpload(false); resetForm(); }} className="flex-1">Cancel</Button>
                <Button type="submit" disabled={drafts.length === 0 || uploading} className="flex-1 gap-2">
                  <Upload className="w-4 h-4" />
                  {uploading ? 'Uploading…' : `Upload ${drafts.length || ''} Photo${drafts.length !== 1 ? 's' : ''}${videoPreview ? ' + Video' : ''}`}
                </Button>
              </div>
            </form>
          </div>
